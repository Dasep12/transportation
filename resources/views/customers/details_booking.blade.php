    ...
    <div class="container py-3">
        <header>
            ...
            <div class="pricing-header p-3 pb-md-4 mx-auto text-center">
                <h1 class="display-6 fw-normal">N&uacute;mero de Reserva: <span class="text-danger">{{ $res[0]->invoice_number  }}</span></h1>
                ...
                <div class="col-lg-7 order-2 order-sm-1">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <hr />
                            <div class="d-flex">
                                <b>Nombre Completo:</b>&nbsp;{{ $res[0]->customer_name }}
                            </div>
                            <div class="d-flex">
                                <b>Tel&eacute;fono:</b>&nbsp;{{ $res[0]->phone_customer == null ? '-' : $res[0]->phone_customer  }}
                            </div>
                            <div class="d-flex">
                                <b>Correo:</b>&nbsp;{{ $res[0]->email }}
                            </div>
                        </div>
                    </div>
                    <br />
                                ...
                                <table class="table ">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($res[0]->photo == null)
                                                <img height="150px" class="img rounded" src="{{ asset('public/images/user.png ') }}" alt="Header Avatar">
                                                @else
                                                <img height="150px" class="img rounded header-profile-user" src="{{ URL::asset ('public/admin/img/driver/' . $res[0]->photo )  }}" alt="">
                                                @endif
                                            </td>
                                        </tr>
                                        <!-- <tr>
                ...
                <div class="col-lg-5  order-1 order-sm-2">
                    <div class="card position-sticky top-0">
                        <div class="p-3 bg-light bg-opacity-10">
                            ...
                            <hr />
                            <div class="d-flex">
                                <b>Quotation Number:</b>&nbsp;{{ $res[0]->invoice_number }}
                            </div>
                            <div class="d-flex">
                                <b>Route Name:</b>&nbsp;{{ $res[0]->route_name }}
                            </div>
                            <div class="d-flex">
                                <b>Departure city :</b>&nbsp;{{ $res[0]->departure_city }}
                            </div>
                            <div class="d-flex justify-content-between mb-1">
                                <b>Departure date and time:</b><span>{{ $res[0]->departure_dates }}</span>
                            </div>
                            <div class="d-flex">
                                <b>Destination City:</b>&nbsp;{{ $res[0]->destination_city }}
                            </div>
                            <div class="d-flex justify-content-between mb-1">
                                <b>Returning date and time:</b><span>{{ $res[0]->returning_dates }}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-1">
                                <b>Car name:</b><span>{{ $res[0]->car_name }}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-1">
                                <b>Passengers:</b><span> {{ $res[0]->passangers_num == null ? '-' : $res[0]->passangers_num  }}</span>
                            </div>


                            <div class="d-flex justify-content-between mb-1">
                                <b>Payment Status:</b>
                                <span class="badge {{  $res[0]->payment_status == 'Partial' ? 'bg-warning' : 'bg-success' }}">{{ $res[0]->payment_status }}</span>
                            </div>

                            <div class="d-flex justify-content-between mb-1">
                                <b>Invoice:</b>
                                @foreach($invoice as $in)
                                <a target="_blank" class="badge bg-danger text-white" href="{{ asset('public/invoice/'. $in->name_invoice) }}">{{ 'Download' }}</a>
                                @endforeach
                            </div>

                            <hr>
                            <div class="d-flex justify-content-between mb-4">
                                <b>Precio Total:</b> <strong class="text-dark">$ {{ number_format($res[0]->payment_total,2) }} MXN</strong>
                            </div>
                            <div class="d-flex justify-content-between mb-4 small">
                                <b>Primer Pago:</b> <span>$ {{ number_format($res[0]->payment_total,2) }} MXN</span>
                            </div>
                            <div class="d-flex justify-content-between mb-4 small">
                                <b>Balance (Pending to pay):</b> <span>$ {{ number_format($res[0]->payment_total - $segundo,2)  }} MXN</span>
                            </div>
                        </div>
                    </div>
                </div>
