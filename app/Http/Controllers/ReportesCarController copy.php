...
class ReportesCarController extends Controller
{
    ...
    public function usoenmes(Request $req)
    {
        ...
        $query = "SELECT c.car_name , IFNULL(sum(X.no_of_days) ,0) as percentage from cars c
        left join (
        ...
        $totaldaysrent = 0;
        foreach (DB::select($query) as $qr) {
            $totaldaysrent += $qr->percentage;
        }

        $datas = array();
        foreach (DB::select($query) as $tr) {
            $datas[] = array(
                'car_name' => $tr->car_name,
                'percentage'    => $tr->percentage != 0 ? number_format(($tr->percentage / $totaldaysrent) * 100, 2) : 0
            );
        }
    ...
    public function tester()
    {
        ...
        // $querys = self::queryDateCars($date1, $date2, $carId);


        $query = DB::select("SELECT c.car_name , IFNULL(sum(X.no_of_days) ,0) as percentage from cars c
        left join (
          select s.date as dates , cps.no_of_days , s.car_id    from services s 
          inner join cost_per_services cps on cps.id = s.costperservices_id 
          where s.payment_status = 'Paid'
    ...
    //         $totalRentDays = DB::select("SELECT c.car_name  , IFNULL(sum(X.no_of_days),0)as res , ROUND(IFNULL((sum(X.no_of_days) / $days) * 100 ,0),2) as percentage from cars c
    //         left join (
    //          select s.date as dates , cps.no_of_days , s.car_id    from services s 
    //           inner join cost_per_services cps on cps.id = s.costperservices_id 
    //           where s.payment_status = 'Paid'
    //         ) X on X.car_id = c.id and date_format(X.dates,'%Y-%m')='$date1'
    //          where c.status  ='Active' and c.id = $c->id  ");


    //         //caro
    //         $caroSQL = DB::select("SELECT s.date , cps.departure_date as startDate , cps.no_of_days as sewa , cps.cars_id , datediff(returning_date, departure_date) as selisih FROM services s LEFT JOIN cost_per_services cps on cps.id = s.costperservices_id WHERE date_format(s.date,'%Y-%m')='$date1' AND cps.cars_id = '$c->id' ");
    //         if (count($caroSQL) > 0) {
    //             foreach ($caroSQL as $cs) {
    //                 for ($h = 0; $h <= $cs->selisih; $h++) {
    //                     $dates = date('Y-m-d', strtotime("+" . $h . " day", strtotime($cs->startDate)));
    //                     array_push($daysCaro, array('dates' => $cs->cars_id . ':' . $dates));
    //                 }
    //             }
    //         }
    ...
    //             }
    //             $skl = $c->id . ':' . $date1 . '-' . $n;
    //             if ($skl == self::searchForId($skl, $daysCaro)) {
    //                 $html .= "<td><i class='bx bx-car font-size-14'></td>";
    //             } else {
    //                 $html .= "<td><i class='fas fa-times text-danger font-size-14'></i></td>";
    //             }
    //         }
    //         $html .= "<td><span class='font-size-13'>" . $totalRentDays[0]->res . "</span></td>";
    //         $html .= "<td><span class='font-size-13'>" . $totalRentDays[0]->percentage . "</span></td>";
    //         $html .= '</tr>';

    //         $totalCars++;
    //         $totalDaysRent += $totalRentDays[0]->res;
    //     }

    //     $totals = ($totalDaysRent / $totalCars / $days) * 100;
    //     $html .= "<div class='col-lg-3'><label>Total</label><br><input readonly class='form-control lg-3' value='" . number_format($totals, 5) . "'></input></div>";
    //     $html .= '</tbody>';
    //     echo $html;
    // }
    ...
    public function showMonths(Request $req)
    {
        ...
        foreach ($cars as $c) {
            ...
            $totalCars++;
            $totals = (self::hitungCars($days) / $totalCars / $hariSebulan) * 100;
    ...
    function searchForId($id, $daysCaro)
    {
        foreach ($daysCaro as $key => $val) {
            if ($val['dates'] === $id) {
                return $val['dates'];
            }
        }
